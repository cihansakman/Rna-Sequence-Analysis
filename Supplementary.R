...
working_dir = "/media/data02/cihan2/de/ballgown/ref_only" 
setwd(working_dir)
...
data_colors = c("#F86624","#F85A12","#ED5007","#DA4A07","#F97639","#F9844D","#D4E4BC","#D0E2B6","#C6DCA7","#BDD699","#B3D08A","#AACB7C")

#View expression values for the transcripts of a particular gene symbol of chromosome 22.  e.g. the Top Significant Gene 'CEP55'
#First determine the rows in the data.frame that match 'CEP55', aka. ENSG00000138180, then display only those rows of the data.frame
i = row.names(gene_expression) == "ENSG00000138180"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once for instance top 4 significant genes? 
#genes_of_interest = c("CEP55", "DSG3", "TROAP", "KIF14")
genes_of_interest = c("ENSG00000138180","ENSG00000134757","ENSG00000135451","ENSG00000118193")
...
data_columns=c(1:12)
short_names=c("SRR3474779","SRR3474795","SRR3474918","SRR3474972","SRR3475081","SRR3475089","SRR3475326","SRR3475332","SRR3475345","SRR3475362","SRR3475375","SRR3475377")
...
boxplot(log2(gene_expression[,data_columns]+min_nonzero), col=data_colors, names=short_names, las=2, ylab="log2(FPKM)", main="Distribution of FPKMs for all 12 Samples")
...
x = gene_expression[,"FPKM.SRR3474779"]
y = gene_expression[,"FPKM.SRR3474795"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (LUSC, SRR3474779)", ylab="FPKM (LUSC, SRR3474795)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (LUSC, SRR3474779)", ylab="FPKM (LUSC, SRR3474795)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
#Plot #6 - Now make a call to our custom function created above, once for each sample comparison
plotCor("FPKM.SRR3475081", "FPKM.SRR3475375", "LUSC-1 vs NORMAL-1", "tomato2")
plotCor("FPKM.SRR3474795", "FPKM.SRR3475332", "LUSC-2 vs NORMAL-2", "royalblue2")
...
plotCor2("FPKM.SRR3475081", "FPKM.SRR3475375", "LUSC-1 vs NORMAL-1", "tomato2")
plotCor2("FPKM.SRR3474795", "FPKM.SRR3475332", "LUSC-2 vs NORMAL-2", "royalblue2")

#Calculate the FPKM sum for all 12 samples
...
par(mfrow=c(1,1))
plot(mds$points, type="n", xlab="", ylab="", main="MDS distance plot (all non-zero genes)", xlim=c(-0.5,0.5), ylim=c(-0.5,0.5))
points(mds$points[,1], mds$points[,2], col="grey", cex=2, pch=16)
text(mds$points[,1], mds$points[,2], short_names, col=data_colors)
###WE WOULD LIKE TO SEE IN THE PLOT THE LUSC SAMPLES AND NORMAL SAMPLES GROUPED TOGETHER
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) LUSC vs NORMAL", main="Distribution of differential expression values")
abline(v=-2, col="black", lwd=2, lty=2)
abline(v=2, col="black", lwd=2, lty=2)
legend("topleft", "Fold-change > 4", lwd=2, lty=2)

#### Plot #10 - Display the grand expression values from LUSC and NORMAL and mark those that are significantly differentially expressed
gene_expression[,"LUSC"]=apply(gene_expression[,c(1:6)], 1, mean)
gene_expression[,"NORMAL"]=apply(gene_expression[,c(7:12)], 1, mean)

x=log2(gene_expression[,"LUSC"]+min_nonzero)
y=log2(gene_expression[,"NORMAL"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="LUSC FPKM (log2)", ylab="NORMAL FPKM (log2)", main="LUSC vs NORMAL FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
...
#### Plot #11 - Create a heatmap to vizualize expression differences between the 12 samples
